...
Codex 产品需求原型工具。安装后可以在 Codex 中使用 `$prd` 或“产品需求工具”触发中文 PRD、BDD、流程图和 Figma 原型工作流。
...
- 调用本仓库的本地安装脚本。
- 安装 `product-requirement-prototyping` 技能和 `$prd` 短别名。
- 如已有旧版本，默认先备份再安装。
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\install-product-requirement-prototyping.ps1
```
